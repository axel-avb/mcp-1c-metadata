...
The full `tools/list` for all 23 tools is **~16,800 chars** (~4.2K tokens for
Latin text). This is loaded into the client context on every start/re-init.

Most of the volume is the JSON schema (generated by FastMCP automatically), not
docstrings. In `pre-4-cut` the verbose value enumerations (`sections`,
`element_type`, `search_by`, type lists) were trimmed from docstrings — they
duplicated the schema; ~1K chars saved with no functional loss.
