...
1. Внешний бинарный парсер легаси-форм (phase-2) — контракт в `xml_object.parse_legacy_object`.
2. Дополнить `.txt` отсутствующими типами объектов (business_process/common_module) из манифеста.
3. Полный переэмбеддинг символов после правки `stable_id`/парсеров (если менялись).
